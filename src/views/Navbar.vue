		...
		<nav class="shadow-sm navbar navbar-expand-lg navbar-light bg-light fixed-top">
			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav mr-auto">
					...
					-->
					<li class="nav-item" v-if="!getUserStatus"><router-link to="/login" class="nav-link">Login</router-link></li>
				</ul>
				<ul class="navbar-nav">
							...
							<div class="dropdown-menu dropdown-menu-right p-0" :style="{ width: isMobile ? '95%' : '' }">
								<ul class="list-group list-group-flush" id="pendingRequests">
									...
									<li class="dropdown-item list-group-item" style="cursor: pointer" v-for="request in pendingRequests" :key="request.id">
										<img class="px-1" style="height: 2em" :src="request.requesterImg" alt="" /> {{ request.requesterName }}
				...
				<ul class="navbar-nav">
					<li class="nav-item notify-dropdown" v-if="getUserStatus">
						<div class="dropdown" @click="notiRead();">
							...
							<div class="dropdown-menu dropdown-menu-right p-0" :style="{ width: isMobile ? '95%' : '' }">
								<ul class="list-group list-group-flush notify-chat" id="notify">
									<h5 class="dropdown-header" v-if="notify.length > 0">New</h5>
									<li class="list-group-item list-group-item-action list-group-item-light" style="cursor: pointer" v-for="noti in friendNotify" :key="noti.id">
										{{ noti.name }} rejected your friend request.
									</li>
									<li class="list-group-item list-group-item-action list-group-item-light d-flex" style="cursor: pointer" v-for="noti in notify" :key="noti.id" @click="show(noti.postId);">
										<img class="px-1 m-0 rounded-circle" style="height: 3em" :src="noti.userImg" alt="" />
										<span class="flex-grow-1">
											<span class="text-primary">{{ noti.user }} </span> {{ getType(noti.type) }} <span class="badge badge-light"> {{ cvDate(noti.createdTime) }}</span>
										</span>
									</li>
									<h5 class="dropdown-header">Old</h5>
									<li class="list-group-item list-group-item-action list-group-item-light d-flex" v-for="noti in inactiveNotify" :key="noti.id" @click="show(noti.postId);">
										<div><img class="px-1 m-0 rounded-circle" style="height: 3em" :src="noti.userImg" alt="" /></div>
					...
					<li class="nav-item notify-dropdown" v-if="getUserStatus">
						<div class="dropdown">
							...
							<div class="dropdown-menu dropdown-menu-right p-0" :style="{ width: isMobile ? '95%' : '' }">
								<ul class="list-group list-group-flush notify-chat" id="notifyChat">
									<li
										class="list-group-item list-group-item-action list-group-item-light py-1 rounded px-2"
										style="cursor: pointer"
										v-for="chat in chatList"
										:key="chat.id"
										:class="{ 'bg-light': chat.isNotified != userInfo.name && chat.hasNew }"
										v-if="chat.membersName && chat.content.length"
										@click="readChat(chat);"
									>
										<img class="px-1 float-left rounded-circle" style="height: 3em" :src="chat.membersImg[getMember(chat.membersEmail)]" />
										<a>
											<a class="text-primary">{{ chat.membersName[getMember(chat.membersEmail)] }}</a>
											<p class="mb-1 text-secondary d-flex justify-content-between" style="max-width: 500px">{{ getChatOwner(chat.content) }} {{ shortenMessage(chat.content) }}</p>
											<span class="badge badge-light text-secondary">{{ calendarTime(chat.time) }}</span>
										</a>
									</li>
								</ul>
							</div>
					...
					<li class="nav-item dropdown" v-if="getUserStatus">
						<a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown"> {{ getUsername }} </a>
...
<script>
import { GET_NOTIFICATION, REMOVE_USER, NOTIFY_READ, GET_POST, GET_INACTIVE_NOTIFICATION, GET_CHAT_LIST, CHAT_NOTIFY_READ, GET_FRIENDS_POST } from "../store/actions.type";
...
export default {
		...
		chatList() {
			...
			this.chatList.forEach(chat => {
				if (chat.isNotified != this.userInfo.name && chat.hasNew == true) {
					a++;
					let audio = new Audio("https://facebook.design/public/sounds/Notification%201.mp3");
					audio.preload = "auto";
					setTimeout(() => {
						audio.play();
					}, 250);
	...
	computed: {
		...mapState({
			...
			inactiveNotify: state => (state.auth.inactiveNoti ? state.auth.inactiveNoti : []),
